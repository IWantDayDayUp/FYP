# src/ecg_fm/training/mae_multidb.py
...
from ecg_fm.data.datasets import build_multidb_dataset
...
def build_mae_multidb_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more .npy files (or paths relative to --data-root)",
    )
    parser.add_argument(
        "--data-root",
        type=str,
        default=os.environ.get("FYP_DATA_DIR", ""),
        help="Optional data root for all npy files",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Limit number of samples per DB (0 = no limit)",
    )
...
def train_mae_multidb(args: argparse.Namespace) -> None:
    ...
    run_name = args.run_name or time.strftime("%Y-%m-%d_%H%M_mae_multidb")
    ...
    # dataset: build from multiple npy files
    limit_per_db = args.limit_per_db if args.limit_per_db > 0 else None
    npy_list: List[str] = args.npy

    log(f"Building MultiDB dataset from {len(npy_list)} files:")
    for p in npy_list:
        log(f"  - {p}")

    ds = build_multidb_dataset(
    ...
    # estimate total npy size (best-effort)
    npy_paths_resolved: list[Path] = []
    total_size = 0
    ...
    for p in npy_list:
        ...
        if root is not None and not pth.is_absolute():
            pth = root / pth
        npy_paths_resolved.append(pth)
        if pth.exists():
            total_size += pth.stat().st_size

    data_info = {
        "npy_paths": [str(p) for p in npy_paths_resolved],
        "npy_total_size_mb": round(bytes_to_mb(total_size), 2),
    ...
    log(
        f"N={data_info['num_samples']}, "
        f"L={L}, npy_total_size_mb={data_info['npy_total_size_mb']}"
    )
    ...
    if L % args.patch_size != 0:
        log(
            ...
            "Training may fail unless you changed model/padding."
        )
    ...
    log("[Done] Finished MultiDB training and saved checkpoints/metrics/meta/summary.")
